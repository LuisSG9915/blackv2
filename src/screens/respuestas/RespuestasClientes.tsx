...
function RespuestasClientes() {
  ...
  const [form, setForm] = useState<Respuestas>({
    id: 0,
    respuestaCliente: "",
  });
  ...
  const [data, setData] = useState<Respuestas[]>([]);

  const mostrarModalActualizar = (dato: Respuestas) => {
    // Assuming 'dato.marca' is of type 'string | undefined'
    setForm({ ...dato, respuestaCliente: dato.respuestaCliente !== undefined ? dato.respuestaCliente : "" });
    console.log(dato);
  ...
  const validarCampos = () => {
    const camposRequeridos: (keyof Respuestas)[] = ["respuestaCliente"];
    const camposVacios: string[] = [];

    camposRequeridos.forEach((campo: keyof Respuestas) => {
  ...
  const insertar = async () => {
    const permiso = await filtroSeguridad("CREAR_RESPUESTA_ADD");
    if (permiso === false) {
    ...
    if (validarCampos() === true) {
      await jezaApi
        .post("/sp_respuestaReagendadoAdd", null, {
          params: {
            respuestaCliente: form. respuestaCliente,
          },
        })
        .then((response) => {
          Swal.fire({
        ...
        .catch((error) => {
          ...
          Swal.fire({
            icon: "error",
            title: "Error",
            text: "Hubo un problema al actualizar. Por favor, intenta nuevamente.",
  ...
  const editar = async () => {
    const permiso = await filtroSeguridad("CAMBIO_RESPUESTA_UPD");
    if (permiso === false) {
      return; // Si el permiso es falso o los campos no son válidos, se sale de la función
    }
    if (validarCampos() === true) {
      await jezaApi
        .put(`/sp_respuestaReagendadoUpd`, null, {
          params: {
            id: form.id,
            respuestaCliente: form. respuestaCliente,
          },
        ...
        .then((response) => {
          Swal.fire({
            icon: "success",
            text: "Respuesta actualizada con éxito",
            confirmButtonColor: "#3085d6",
          });
  ...
  const eliminar = async (dato: Respuestas) => {
    const permiso = await filtroSeguridad("ELIM_RESPUESTA_DEL");
    if (permiso === false) {
      return; // Si el permiso es falso o los campos no son válidos, se sale de la función
    }
    Swal.fire({
      title: "ADVERTENCIA",
      text: `¿Está seguro que desea eliminar el registro: ${dato.respuestaCliente}?`,
      icon: "warning",
      ...
      if (result.isConfirmed) {
        jezaApi.delete(`/sp_respuestaReagendadoDel?id=${dato.id}`).then(() => {
          Swal.fire({
            icon: "success",
  ...
  const getMarca = () => {
    jezaApi.get("/sp_respuestaClienteReagendado?id=0").then((response) => {
  ...
  const columns: GridColDef[] = [
    ...
    { field: "respuestaCliente", headerName: "Respuestas del cliente", width: 1000, headerClassName: "custom-header" },
  ];
  ...
  return (
    <>
        ...
        <Row>
          <Container fluid>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <h1> Respuestas clientes <AiFillTags size={35}></AiFillTags></h1>
      ...
      <Modal isOpen={modalInsertar} size="xl">
        ...
        <ModalBody>
          <Container>
            <CFormGroupInput handleChange={handleChange} inputName="respuestaCliente" labelName="Respuesta:" value={form.respuestaCliente} minlength={1} maxlength={50} />
          </Container>
        </ModalBody>
        <ModalFooter>
          <CButton color="success" onClick={insertar} text="Guardar" />
...
export default RespuestasClientes;

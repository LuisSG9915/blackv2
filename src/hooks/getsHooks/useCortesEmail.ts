...
export const useCortesEmail = ({ sucursal, fecha }: Props) => {
  ...
  const fetchCorteA = async () => {
    try {
      const response: AxiosResponse<any[]> = await jezaApi.get(`/CorteSeccionA?suc=${sucursal}&fecha=${fecha ? fecha : new Date()}`);
  ...
  const fetchCorteB = async () => {
    try {
      const response: AxiosResponse<any[]> = await jezaApi.get(`/CorteSeccionB?suc=${sucursal}&fecha=${fecha ? fecha : new Date()}`);
  ...
  const fetchCorteC = async () => {
    try {
      const response: AxiosResponse<any[]> = await jezaApi.get(`/CorteSeccionC?suc=${sucursal}&fecha=${fecha ? fecha : new Date()}`);
